...
public class ThematicActivity extends BaseActivity implements Listener<JSONObject> ,ErrorListener,OnPageChangeListener{
	...
	private void request(){
		...
		requestTool.startStringRequest(Method.GET, new Listener<String>() {
			...
			public void onResponse(String arg0) {
				...
				if (thematicInfo.getAlldata().length > 0) {
					setPagerData(thematicInfo.getAlldata());
				}
				if (thematicInfo.getAlldata().length > 3) {
					setData(thematicInfo);
				}
			}
	...
	private void setPagerData(ThematicItem[] items){
		int length = 0;
		if (items.length > 3) {
			length = 3;
		}else {
		...
		for(int i = 0;i < length;i++){
			...
			StoreApplication.getInstance().getImageLoader().get(items[i].getFocus(), listener);
			topImgs[i] = img;
		}
