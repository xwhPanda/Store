 ...
 *******************************************************************************/
package com.luc.muldownload.widget;
...
import com.luc.muldownload.widget.internal.FlipLoadingLayout;
import com.luc.muldownload.widget.internal.LoadingLayout;
import com.luc.muldownload.widget.internal.RotateLoadingLayout;
import com.luc.muldownload.widget.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public static final int DEFAULT_SMOOTH_SCROLL_HEIGHT = 100;
	...
	@Override
	public final void onRefreshComplete(boolean doUpadteTime) {
		if (isRefreshing()) {
		...
		if (doUpadteTime) {
			...
			String label = DateFormat.format("æœ?Žæ›´æ–° :ä»Šå¤©  kk:mm:ss", System.currentTimeMillis()).toString();
	...
	public final void setRefreshing(boolean doScroll) {
		if (!isRefreshing()) {
			if (mMode==Mode.BOTH && mCurrentMode!=Mode.PULL_FROM_START) {
				mCurrentMode = Mode.PULL_FROM_START;				
			}
	...
	protected final int getFooterSize() {
		int size = mFooterLayout.getContentSize();
		return size == 0 ? DEFAULT_SMOOTH_SCROLL_HEIGHT : size;
	}
	...
	protected final int getHeaderSize() {
		int size = mHeaderLayout.getContentSize();
		return size == 0 ? DEFAULT_SMOOTH_SCROLL_HEIGHT : size;
	}
	...
	protected void onRefreshing(final boolean doScroll) {
		...
		if (doScroll) {
			if (mShowViewWhileRefreshing) {
				...
				case PULL_FROM_END:
					smoothScrollTo(getFooterSize(), mState == State.MANUAL_REFRESHING ? null : listener);
					break;
				default:
				case PULL_FROM_START:
					smoothScrollTo(-getHeaderSize(), mState == State.MANUAL_REFRESHING ? null : listener);
					break;
				}
			} else {
				smoothScrollTo(0);
